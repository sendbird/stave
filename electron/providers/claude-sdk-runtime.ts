...
import {
  coerceClaudeElicitationAnswer,
  mapClaudeElicitationToUserInput,
  mapClaudeUserDialogToUserInput,
  parseClaudeQuestionList,
  shouldAutoAcceptClaudeElicitation,
} from "./claude-user-input";
export {
  parseClaudeQuestionList,
  shouldAutoAcceptClaudeElicitation,
} from "./claude-user-input";
...
/** Keep turn-time usage observations owned by the runtime adapter. */
export function mapClaudeMessageToEvents(
  args: Parameters<typeof mapClaudeEvents>[0],
): BridgeEvent[] {
  if (args.message.type === "rate_limit_event" && args.message.rate_limit_info) {
    recordClaudeRateLimitObservation({
      observation: args.message.rate_limit_info,
    });
  }
  return mapClaudeEvents(args);
}
