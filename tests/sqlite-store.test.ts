...
describe("SqliteStore", () => {
  ...
  beforeEach(() => {
    rootDir = path.join(
      tmpdir(),
      `stave-sqlite-store-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  nativeSqliteTest(
    "recovers workspace snapshot and turn summaries across restart",
    async () => {
      ...
      const loadedByWorkspace = Object.fromEntries(
        ["ws-panes", "ws-empty", "ws-legacy"].map((workspaceId) => [
          workspaceId,
          {
            ...
            restoreShell: reopened.loadWorkspaceShellForRestore({
              workspaceId,
            }),
  ...
  nativeSqliteTest(
    "purges legacy turn journals and turn payload artifacts on boot",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
      store.beginTurn({
    ...
    `,
      ...
      expect(
        existsSync(path.join(artifactDir, "legacy-turn-payload.json")),
      ).toBe(false);
    },
  );
  ...
  nativeSqliteTest(
    ...
    async () => {
      ...
      const sinceTwo = store.getStreamEvents({
        turnId: "turn-1",
        sinceSequence: 2,
      });
  ...
  nativeSqliteTest(
    "lists the latest active turn for each task even when a newer completed turn exists",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
  ...
  nativeSqliteTest(
    "loads workspace shell and paged task messages without dropping preserved tasks",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
      ...
      expect(latestTaskOne?.messages.map((message) => message.id)).toEqual([
        "m-2",
      ]);
      expect(latestTaskOne?.totalCount).toBe(2);
      expect(preservedTaskTwo?.messages.map((message) => message.id)).toEqual([
        "m-3",
      ]);
      ...
      expect(
        fullSnapshot?.messagesByTask["task-2"]?.map((message) => message.id),
      ).toEqual(["m-3"]);
  ...
  nativeSqliteTest(
    "preserves unloaded older messages when re-persisting a partial tail window",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
  ...
  nativeSqliteTest(
    "loads a thin workspace shell summary without replaying full shell payloads",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
      ...
      const reopened = new SqliteStore({ dbPath });
      const summary = reopened.loadWorkspaceShellSummary({
        workspaceId: "ws-1",
      });
      reopened.close();
  ...
  nativeSqliteTest(
    "loads a workspace shell lite payload without replaying editor bodies",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
      ...
      const reopened = new SqliteStore({ dbPath });
      const shellLite = reopened.loadWorkspaceShellLite({
        workspaceId: "ws-1",
      });
      reopened.close();
  ...
  nativeSqliteTest(
    "loads restore shells with only the active clean file tab hydrated",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
  ...
  nativeSqliteTest(
    "keeps tasks and messages when a later snapshot omits existing tasks",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
      ...
      expect(taskTwoMessages?.messages.map((message) => message.id)).toEqual([
        "m-2",
      ]);
  ...
  nativeSqliteTest(
    "removes task state only through explicit task removal",
    async () => {
      const SqliteStore = await loadSqliteStore();
      const store = new SqliteStore({ dbPath });
    ...
    async () => {
      ...
      const approval = store.createNotification({
        notification: {
          ...
          expiresAt: "2026-03-06T01:11:30.000Z",
      ...
      expect(approval.notification?.expiresAt).toBeNull();
      ...
      expect(store.clearNotificationHistory()).toBe(2);
  ...
  nativeSqliteTest("stores and clears local MCP request logs", async () => {
    ...
    const firstPage = store.listLocalMcpRequestLogs({
      limit: 1,
      includePayload: false,
    });

    expect(firstPage.logs.map((log) => log.id)).toEqual(["local-mcp-log-2"]);
    ...
    const secondPage = store.listLocalMcpRequestLogs({
      limit: 1,
      offset: 1,
      includePayload: false,
    });

    expect(secondPage.logs.map((log) => log.id)).toEqual(["local-mcp-log-1"]);
    ...
    const clampedPage = store.listLocalMcpRequestLogs({
      limit: 1,
      offset: 99,
      includePayload: false,
    });
    ...
    const log = store.getLocalMcpRequestLog({
      id: "local-mcp-log-1",
      includePayload: true,
    });
    ...
    expect(
      store.listLocalMcpRequestLogs({ limit: 10, includePayload: false }),
    ).toEqual({
      logs: [],
      total: 0,
      limit: 10,
