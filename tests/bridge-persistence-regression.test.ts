...
describe("workspace store hydration ordering", () => {
  ...
  test("runs an instant macro on its pinned model without changing the selected model", async () => {
    const localStorage = createMemoryStorage();
    let startedRequest:
      | {
          providerId?: string;
          prompt?: string;
          runtimeOptions?: { model?: string; claudeEffort?: string };
        }
      | undefined;
    ...
    const selectedRuntime = {
      model: "gpt-5.4",
      modelProviderId: "codex" as const,
    ...
    useAppStore.setState({
      ...
      hasHydratedWorkspaces: true,
      workspaces: [
        { id: "ws-main", name: "Main", updatedAt: "2026-09-10T00:00:00.000Z" },
      ],
      activeWorkspaceId: "ws-main",
      activeTaskId: "task-main",
      projectPath: "/tmp/stave-project",
      ...
      promptDraftByTask: {
        "task-main": {
          ...
          attachedFilePaths: [],
          attachments: [],
          runtimeOverrides: selectedRuntime,
        },
      },
      nativeSessionReadyByTask: {},
      providerSessionByTask: {},
      taskWorkspaceIdById: { "task-main": "ws-main" },
    });

    const macroResult = useAppStore.getState().applyMacroToDraft({
      taskId: "task-main",
      macroId: "macro-review",
      draftText: "!review",
      tokenMatch: { start: 0, end: 7 },
    });
    expect(macroResult).toMatchObject({
      ok: true,
      instantRun: true,
      providerOverride: "claude-code",
      runtimeOverrides: {
    ...
    const started = await useAppStore.getState().sendUserMessage({
      ...
      providerOverride: macroResult.providerOverride,
      runtimeOverrides: macroResult.runtimeOverrides,
