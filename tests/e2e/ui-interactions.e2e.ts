...
async function expectCliSessionCounts(page: Page, expected: CliSessionCounts) {
  await expect
    .poll(async () =>
      page.evaluate(() => ({
        create:
...
test("settings modal and workspace modal open", async ({ page }) => {
  ...
  await expect
    .poll(async () => {
      const [buttonBox, rowBox] = await Promise.all([
        backToApp.boundingBox(),
        backToAppRow.boundingBox(),
      ]);
      return buttonBox && rowBox ? buttonBox.width / rowBox.width : 0;
    })
    .toBeGreaterThan(0.9);
...
test("right panel tabs switch", async ({ page }) => {
  ...
  await expect(
    page.getByTestId("workspace-bar").getByRole("button", { name: "Editor" }),
  ).toHaveCount(0);
  await page
  ...
  await expect(
    rightPanel.getByRole("heading", { name: "Explorer" }),
  ).toBeVisible();
  ...
  await page
    ...
    .click();
  await expect(
    rightPanel.getByRole("heading", { name: "Information" }),
  ).toBeVisible();
});
...
test("cli session keeps the renderer alive and refocuses after switching back", async ({
  page,
}) => {
  await installCliSessionHarness(page);
  ...
  await expect(
    page.getByTestId("cli-session-panel").locator(".xterm"),
  ).toHaveCount(1);
  ...
  await expect
    .poll(async () =>
      terminalSurface.evaluate((element) => {
        ...
        return (
          textarea instanceof HTMLElement && document.activeElement === textarea
        );
      }),
    )
    .toBe(true);
...
test("cli session viewport background follows the active terminal theme token", async ({
  page,
}) => {
  await installCliSessionHarness(page);
  ...
  await expect(page.getByTestId("cli-session-panel")).toBeVisible();
  await expect(
    page.getByTestId("cli-session-panel").locator(".xterm-viewport"),
  ).toHaveCount(1);

  await expect
    .poll(async () => cliViewportMatchesTerminalToken(page))
    .toBe(true);
...
test("cli session resumes streaming after a slow reattach resize", async ({
  page,
}) => {
...
test("scripts manager waits for default scope and keeps draft entries dirty", async ({
  page,
}) => {
  await page.addInitScript(() => {
          ...
          recentProjects: [
            {
              ...
              workspaceBranchById: {
                "ws-default": "main",
                "ws-main": "feat-scripts",
              },
              workspacePathById: {
                "ws-default": "/tmp/stave-project",
                "ws-main": "/tmp/stave-project/.stave/workspaces/feat-scripts",
              },
              workspaceDefaultById: {
                "ws-default": true,
                "ws-main": false,
