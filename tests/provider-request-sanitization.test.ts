...
describe("provider request sanitization", () => {
  test("sanitizes oversized file_context payloads before starting a turn", async () => {
      ...
      api: {
        provider: {
          startPushTurn: async (args: Record<string, unknown>) => {
            startedConversation = args.conversation as
    ...
    useAppStore.setState({
      ...
      workspaces: [
        { id: "ws-main", name: "Main", updatedAt: "2026-03-15T00:00:00.000Z" },
      ],
    ...
    const conversationContextParts = startedConversation?.contextParts as
      Array<Record<string, unknown>> | undefined;
    expect(conversationContextParts?.[0]?.type).toBe("file_context");
    expect(conversationContextParts?.[0]?.content).toBeString();
    expect(String(conversationContextParts?.[0]?.content)).toContain(
      "image payload omitted",
    );
    expect(String(conversationContextParts?.[0]?.content)).not.toContain(
      "data:image/svg+xml;base64",
    );
    expect(
      String(conversationContextParts?.[0]?.content).length,
    ).toBeLessThanOrEqual(MAX_FILE_CONTEXT_CONTENT_CHARS);

    const storedMessagePart =
    ...
    expect(storedMessagePart.content).not.toContain(
      "data:image/svg+xml;base64",
    );
    expect(storedMessagePart.content.length).toBeLessThanOrEqual(
  ...
  test("sanitizes oversized historical tool outputs before starting a follow-up turn", async () => {
    ...
    const { useAppStore } = await import("../src/store/app.store");
    const initialState = useAppStore.getInitialState();
    const oversizedToolOutput = "o".repeat(
    ...
    const history = startedConversation?.history as
      Array<Record<string, unknown>> | undefined;
    const historyParts = history?.[0]?.parts as
      Array<Record<string, unknown>> | undefined;
    expect(historyParts?.[0]?.type).toBe("tool_use");
    expect(String(historyParts?.[0]?.output)).toContain(
      "tool output truncated",
    );
    expect(String(historyParts?.[0]?.output).length).toBeLessThanOrEqual(
      MAX_FILE_CONTEXT_CONTENT_CHARS,
    );
  });
  ...
  test("reattaches task-scoped Crane context after managed takeover", async () => {
    ...
    const contextParts = startedConversation?.contextParts as
      | Array<{
          type?: string;
          sourceId?: string;
          content?: string;
  ...
  test("does not start a follow-up turn while an approval remains pending", async () => {
    ...
    expect(
      useAppStore.getState().activeTurnIdsByTask["task-1"],
    ).toBeUndefined();
  });
  ...
  test("strips renderer-only tool metadata from historical tool parts before follow-up turns", async () => {
    ...
    const history = startedConversation?.history as
      Array<Record<string, unknown>> | undefined;
    const historyPart = history?.[0]?.parts as
      Array<Record<string, unknown>> | undefined;
  ...
  test("compacts oversized provider turn requests below the host-service stdin budget", () => {
    ...
    expect(bounded.conversation?.history.length).toBeLessThan(
      request.conversation.history.length,
    );
    expect(
  ...
  test("keeps resume history until the runtime confirms the native session", () => {
    ...
    expect(bounded.conversation?.resume?.syncedThroughMessageId).toBe(
      "history-4",
    );
  ...
  test("keeps current task awareness and file context before lower-priority context", () => {
      ...
      conversation: {
        ...
        contextParts: [
          ...
          },
          {
            ...
            language: "ts",
            instruction:
              "Prefer this file if you need concrete implementation details.",
    ...
    expect(sourceIds).not.toContain("stave:test-retrieved-context");
    expect(
      bounded.conversation?.contextParts.some(
        (part) => part.type === "file_context",
      ),
    ).toBe(true);
  });
      ...
      api: {
        provider: {
          streamTurn: async (args: Record<string, unknown>) => {
            ...
            if (calls.length === 1) {
              throw new Error(
                "[host-service] provider.stream-turn request exceeded protocol message limit (1048577 bytes > 1048576)",
              );
            }
            return [{ type: "text", text: "retried" }, { type: "done" }];
          },
        },
      },
    };
