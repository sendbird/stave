...
type FileContextPayload = Pick<
  FileContextPart,
  "type" | "filePath" | "content" | "language" | "instruction"
>;
...
export function sanitizeTextField(args: {
  value: string;
  label: string;
  maxChars?: number;
}) {
...
export function sanitizeFileContextPayload<T extends FileContextPayload>(
  part: T,
): T {
  const instruction =
  ...
  if (
    part.content.length <= MAX_FILE_CONTEXT_CONTENT_CHARS &&
  ...
  const content =
    part.content.length <= MAX_FILE_CONTEXT_CONTENT_CHARS
      ...
      : isImageFileContext(part)
        ? buildOmittedImagePayloadNotice({
      ...
      const changed = questions.some(
        (question, questionIndex) =>
          ...
          question.defaultValue !==
            part.questions[questionIndex]?.defaultValue ||
          question.linkUrl !== part.questions[questionIndex]?.linkUrl ||
          question.options.some(
            (option, optionIndex) =>
              option.label !==
                part.questions[questionIndex]?.options[optionIndex]?.label ||
              option.description !==
                part.questions[questionIndex]?.options[optionIndex]
                  ?.description,
          ),
      );
