...
# Martin ↔ Stave Workspace Sync — 설계 문서
...
Stave workspace(git worktree 단위의 로컬 에이전트 작업 공간)를 Atelier의 Martin
project(기능 런칭 단위의 팀 가시적 변경 추적 프로젝트)에 연결하고 양방향으로 동기화한다.

- **풀 (Martin → Stave)**: 프로젝트 요약·일정·제약·gotcha·최근 변경 이벤트를
  워크스페이스로 가져와 에이전트를 그라운딩한다. (Martin 제품 문서의
  "Agents" 시나리오를 구현)
- **푸시 (Stave → Martin)**: Stave에서 일어난 작업(PR 오픈, 태스크 완료,
  리소스 링크, opt-in 턴 요약)을 Martin의 change event / links로 올려
  팀 가시성을 만든다. Martin이 `stave`라는 새 변경 소스를 얻는다.
...
|---|---|
| 방향 | 양방향 (매핑 + 상호 싱크) |
| 매핑 단위 | Stave workspace N : 1 Martin project, 수동 연결 |
...
- Martin → Stave 실시간 알림/배지 (stamp 폴링 기반, Phase 2 후보)
- 커넥터 스코프 업그레이드 엔드포인트 (기존 Crane 커넥터 사용자는 재페어링)
- Stave todos ↔ Martin schedule stages 동기화 (의미 불일치, 보류)
- Martin 웹 UI에서 Stave workspace를 조회/제어하는 기능
...
- **Martin project**: `martin_projects` 행 + 5개 섹션(members, links,
  properties, schedule stages, memory entries) + `martin_change_events`
  원장 + `martin_context_documents`.
- **매핑**: N개의 Stave workspace → 1개의 Martin project. 매핑의 소유자는
  **Stave** (Workspace Information에 저장). Martin 쪽에는 매핑 테이블을
...
│       martin-sync (outbox 런타임)│ Bearer   │ martin: /api/martin/stave/*│
...
- `resolveStaveConnectorCaller`를 `apps/crane/src/server/stave-dispatch-auth.mjs`에서
  플랫폼 공용 모듈 `src/stave-connector/auth.mjs`로 추출. Crane과 Martin
  라우트가 같은 resolver를 사용하되, 호출마다 다음을 라이브로 재확인한다:
  1. 커넥터 시크릿(`stc_`) 해시 일치 + 사용자 활성 상태 (기존 동작)
  2. 요청 라우트가 요구하는 **스코프** 포함 여부 (`crane` / `martin`)
  3. 해당 앱 권한 (`martin:view` 또는 `martin:edit`)
- 페어링 교환(`POST /api/crane/stave/connectors/exchange` — 경로 유지)에
  `requestedScopes` 필드 추가. 서버는 사용자가 실제 가진 권한과의 교집합만 부여.
- 기존 커넥터는 crane 전용으로 남는다. Martin 스코프가 필요하면 재페어링.

### 4.2 Martin Stave 라우트 (신설)

파일: `apps/martin/src/server/stave-sync-routes.mjs`
(등록: `routes.mjs`의 `registerMartinRoutes`에서 위임)

| 라우트 | 스코프/권한 | 용도 |
|---|---|---|
| `GET /api/martin/stave/projects?query=&limit=` | `martin` + `martin:view` | 매핑 피커용 목록. visibility 존중 (personal은 커넥터 소유자만) |
| `GET /api/martin/stave/projects/:ref/context-bundle` | 〃 | 프로젝트 + 5개 섹션 + 최근 change event 50개 + markdown 프로젝션 일괄 반환 |
| `POST /api/martin/stave/projects/:ref/events` | `martin` + `martin:edit` | change event 배치 푸시 (멱등) |
| `POST /api/martin/stave/projects/:ref/links/merge` | 〃 | links 섹션 서버 측 병합 |

공통 정책:

- `:ref`는 기존 라우트와 동일하게 slug 또는 id 허용, `resolveProject`의
  visibility 체크 재사용.
- Feature flag `MARTIN_STAVE_SYNC_ENABLED` — off면 라우트 전체 404.
- 모든 쓰기는 `platform_audit_logs` 기록 (`appSlug: "martin"`).
...
- 중복 방지: `martin_change_events`에 표현식 부분 유니크 인덱스
...
- 마이그레이션으로 `martin_links`에 `origin TEXT` nullable 컬럼 추가
...
- 규칙 (단일 `db.batch`로 원자 처리):
  ...
  - **삭제는 절대 하지 않음** (Stave에서 리소스를 제거해도 Martin 링크는 유지).
...
2. `martin_stave_sync` —
   - `martin_change_events.source` CHECK에 `stave` 추가 (SQLite 특성상
     테이블 리빌드: 새 테이블 생성 → 복사 → 스왑).
   - `staveEventId` 표현식 유니크 인덱스.
   - `martin_links.origin` 컬럼.
...
### 5.2 싱크 엔진: `electron/main/martin-sync/`

- **Durable outbox**: 새 SQLite 테이블 `martin_sync_outbox`
...
| 소스 | 경로 |
|---|---|
| `pr.afterOpen`, `task.archiving`, `turn.completed` | renderer script-trigger 디스패처 지점에서 `martin-sync:enqueue` IPC |
...
- **매핑**: `WorkspaceInformationState`에 1급 필드 추가 —
  `martinProject?: { ref, slug, name, url, linkedAt, lastPulledAt }`.
  스냅샷에 함께 저장되고 Information panel에 노출.
- **풀 스냅샷**: context-bundle의 markdown을 워크스페이스의
  `.stave/context/martin/<slug>.md`에 기록. 에이전트가 파일로 자연스럽게
...
- `stave_martin_list_projects` — 검색어로 프로젝트 목록 (피커/에이전트 공용)
- `stave_martin_link_project` / `stave_martin_unlink_project`
- `stave_martin_get_context` — on-demand 최신 번들 조회 + 스냅샷 파일 갱신

### 5.6 설정 & UI

- `AppSettings.martinSync`: 마스터 스위치 + 이벤트 종류별 토글.
...
- 설정 integrations 섹션: 기존 Crane 카드 옆에 "Atelier 커넥터" 상태
  (페어링/스코프/last seen) + Martin 싱크 토글 + outbox 상태(대기/실패 건수).
- Information panel: **Martin 카드** — 연결된 프로젝트명(클릭 시 웹 열기),
...
### 6.2 푸시: 리소스 링크 → `martin_links`

| Stave WI 리소스 | Martin `kind` | label 규칙 |
|---|---|---|
...
- 번들 구성: project(name/slug/summary/status/updatedAt) + members + links +
  ...
  markdown 프로젝션.
- 저장: markdown → `.stave/context/martin/<slug>.md`,
  메타(`lastPulledAt` 등) → WI `martinProject` 필드.
- Stave WI의 리소스 섹션으로의 역병합(예: Martin links → Stave Jira 목록)은
...
### Atelier (`apps/martin/tests/`, `tests/test-db.ts` 하니스)
...
- 계약 fixture: `apps/martin/tests/fixtures/stave-sync-v1/*.json`.

### Stave (`tests/martin-*.test.ts`)
...
3. 문서: Stave `docs/features/martin-sync.md` (feature doc 게이트),
   Atelier `docs/apps/martin.md` 및 커넥터 문서 갱신.

## 11. 미해결 질문 / Phase 2 후보

- 커넥터 스코프 업그레이드 엔드포인트 (재페어링 없이 스코프 추가).
- Martin 변경 감지 배지 (stamp 폴링) — 연결된 프로젝트가 바뀌면
  Information panel에 "새 변경 N건" 표시.
- 턴 요약 스로틀/배치 (turnSummaries를 켠 사용자의 피드 노이즈 완화).
- Martin MCP (Atelier Phase 3)가 생기면 `stave_martin_get_context`의
  역할 재검토 (중복 가능).
- Stave todos ↔ Martin schedule/memory 매핑.
