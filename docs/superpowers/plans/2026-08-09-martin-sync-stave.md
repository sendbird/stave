# Martin Sync (Stave) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the Stave half of Martin ↔ Stave workspace sync: generalize the Crane connector into an Atelier connector (scoped credentials, OS-encrypted vault, one-time migration), add a durable SQLite outbox + retrying sync runtime that pushes workspace events and resource links to Martin, pull project context bundles into `<worktree>/.stave/context/martin/<slug>.md`, expose 4 MCP tools, and surface settings + Information-panel UI. Assumes the Atelier server API (spec §4) already exists.

**Architecture:** Renderer trigger points (`pr.afterOpen`, task archiving, turn summaries, WI edits) send `martin-sync:*` IPC to Electron main. Main owns credentials (`electron/main/atelier-connector/`, vault at `userData/atelier-connector.v1.json`, migrated once from `crane-connector.v1.json`) and the sync engine (`electron/main/martin-sync/`): synchronous enqueue into the `martin_sync_outbox` SQLite table, then a Crane-style serialized operation queue with generation-guarded `setTimeout` and exponential backoff drains it against 4 Atelier HTTPS endpoints. Agent-driven WI changes are observed in main via the existing `local-mcp.workspace-information-updated` host-service event. The workspace ↔ project mapping is a first-class `martinProject` field on `WorkspaceInformationState`, mutated only through a new host-service local-mcp action so all consumers (renderer panel, MCP tools, persistence) stay consistent. Contract `stave-sync-v1` is a shared Zod module with JSON fixtures duplicated in both repos (same pattern as `stave-dispatch-v1`).
...
- Per-task gate: `bun run typecheck` + the task's focused `bun test tests/martin-*.test.ts` must pass before commit.
- `src/store/app.store.ts` is ratcheted at 3148 lines (currently 3108) — keep total additions there under ~25 lines; all logic lives in `src/lib/martin-sync/renderer-triggers.ts`.
- Any `switch` over the new unions (`MartinOutboxKind`, `MartinOutboxStatus`, `StaveSyncEventKind`) must be exhaustive (`default` returning `never`-checked value) so `check:switch-exhaustiveness` conventions hold.
- The contract version literal is `"stave-sync-v1"` — it appears only in `src/lib/martin-sync/contract.ts` and fixtures; everything else imports it.
...
**Files:**
- Create: `src/lib/martin-sync/contract.ts`
- Create: `src/lib/martin-sync/types.ts`
- Create: `src/lib/martin-sync/links.ts`
...
- Test: `tests/martin-sync-contract.test.ts`, `tests/martin-sync-settings.test.ts`

**Interfaces:**
- Consumes: `WorkspaceInformationState` from `src/lib/workspace-information.ts` (links.ts only).
- Produces: `STAVE_SYNC_CONTRACT_VERSION`, `StaveSyncEventV1Schema`, `StaveSyncEventsRequestV1Schema`, `StaveSyncEventsResponseV1Schema`, `StaveSyncLinkV1Schema`, `StaveSyncLinksMergeRequestV1Schema`, `StaveSyncLinksMergeResponseV1Schema`, `MartinProjectRowV1Schema`, `MartinProjectListResponseV1Schema`, `MartinContextBundleV1Schema`, `toMartinProjectSummary(row, baseUrl): MartinProjectSummary`; `MartinSyncSettingsSchema`, `DEFAULT_MARTIN_SYNC_SETTINGS`, `normalizeMartinSyncSettings(value: unknown): MartinSyncSettings`; `buildMartinSyncLinks(info: WorkspaceInformationState): StaveSyncLinkV1[]`.

- [ ] **Step 1: Write the failing tests**

`tests/martin-sync-contract.test.ts` (model: `tests/crane-stave-contract.test.ts`):
...
`tests/martin-sync-settings.test.ts`:
...
`apps/martin/tests/fixtures/stave-sync-v1/` (authored in the Atelier plan, Task 4);
copy them verbatim rather than re-authoring — the duplicated fixtures are the
cross-repo contract check.

- [ ] **Step 2: Run tests to verify they fail** — `bun test tests/martin-sync-contract.test.ts tests/martin-sync-settings.test.ts`; expect `error: Cannot find module '../src/lib/martin-sync/contract'`.

- [ ] **Step 3: Write minimal implementation**

`src/lib/martin-sync/contract.ts` (model: `src/lib/crane-connector/contract.ts` — trimmed strings, `.strict()`, byte-free limits object):
...
// Mirrors the SERVER module apps/martin/src/server/stave-sync-contract.mjs
...
// Atelier repo's apps/martin/tests/fixtures/stave-sync-v1/.
...
// Full server enum (martin_links.kind); Stave only emits a subset
...
// the web URL and the stable ref client-side via toMartinProjectSummary.
...
export const MartinProjectListResponseV1Schema = z
  .object({
    contract: z.literal(STAVE_SYNC_CONTRACT_VERSION),
    projects: z.array(MartinProjectRowV1Schema),
  })
  .strict();
...
export type MartinProjectSummary = {
  ...
  summary: string;
  url: string; // `${baseUrl}/apps/martin/p/${slug}`
  updatedAt: string;
};

export function toMartinProjectSummary(
  row: MartinProjectRowV1,
  baseUrl: string,
...
export type MartinProjectRowV1 = z.infer<typeof MartinProjectRowV1Schema>;
export type MartinContextBundleV1 = z.infer<typeof MartinContextBundleV1Schema>;
```

`src/lib/martin-sync/types.ts` — `MartinSyncSettingsSchema` strict object of the five booleans, `DEFAULT_MARTIN_SYNC_SETTINGS` frozen, `normalizeMartinSyncSettings` = safeParse, else per-key `typeof x === "boolean"` salvage over defaults (mirror `normalizeCraneConnectorSettings` in `src/lib/crane-connector/types.ts:294`). Also define the IPC arg schemas used later (kept here so `electron/main/ipc/schemas.ts` can re-export, same as crane):
...
`src/lib/martin-sync/links.ts` — pure mapping per spec §6.2, ordered PRs → figma → slack → jira → confluence → storybook → amplify; skip entries whose `url` fails `StaveSyncLinkV1Schema` https check; labels: PR title (fallback url), figma/storybook/confluence/amplify title/label, slack `channelName` fallback url, jira `` `${issueKey}: ${title}` ``.

- [ ] **Step 4: Run tests to verify they pass** — `bun test tests/martin-sync-contract.test.ts tests/martin-sync-settings.test.ts` and `bun run typecheck`.
- [ ] **Step 5: Commit** — `git add src/lib/martin-sync tests/martin-sync-contract.test.ts tests/martin-sync-settings.test.ts tests/fixtures/stave-sync-v1 && git commit -m "feat(martin-sync): add stave-sync-v1 contract, settings types, and link mapping" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
- Test: `tests/martin-atelier-vault.test.ts`
...
- [ ] **Step 1: Write the failing test** — `tests/martin-atelier-vault.test.ts` modeled on `tests/crane-connector-credential-vault.test.ts` (fake crypto `encrypted:` prefix, tmpdir per test):
  - save/get round-trip preserves `scopes: ["crane", "martin"]`;
...
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-atelier-vault.test.ts`; expect `Cannot find module '../electron/main/atelier-connector/credential-vault'`.
...
- [ ] **Step 4: Run tests to verify pass** — `bun test tests/martin-atelier-vault.test.ts tests/crane-connector-credential-vault.test.ts tests/crane-connector-runtime.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add src/lib/atelier-connector electron/main/atelier-connector electron/main/crane-connector tests/martin-atelier-vault.test.ts && git commit -m "feat(atelier-connector): scoped credential vault with one-time crane vault migration" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`

---

### Task 3: Atelier HTTP client (pairing with `requestedScopes` + 4 Martin endpoints)

**Files:**
- Create: `electron/main/atelier-connector/http-client.ts`
- Test: `tests/martin-http-client.test.ts`
...
export class AtelierConnectorHttpClient {
  ...
  listMartinProjects(args: { secret: string; query?: string; limit?: number; signal?: AbortSignal }): Promise<MartinProjectSummary[]>;
  getMartinContextBundle(args: { secret: string; projectRef: string; signal?: AbortSignal }): Promise<MartinContextBundleV1>;
  postMartinEvents(args: { secret: string; projectRef: string; events: StaveSyncEventV1[]; signal?: AbortSignal }):
    Promise<Array<{ staveEventId: string; status: "inserted" | "duplicate" }>>;
  mergeMartinLinks(args: { secret: string; projectRef: string; links: StaveSyncLinkV1[]; signal?: AbortSignal }):
...
- [ ] **Step 1: Write the failing test** — model `tests/crane-connector-http-client.test.ts` style with an injected `fetch` stub returning `new Response(JSON.stringify(...), { status, headers })`:
  - `exchangePairingCode` sends `requestedScopes` in the JSON body and parses granted scopes from **`connector.scopes`** in the response (`{ connector: { …, scopes: ["crane","martin"] }, secret: "stc_x", pollRetryMs: 15000 }` — the Atelier route returns the connector row, which carries `scopes`); when `connector.scopes` is absent (pre-scopes server) it defaults to `["crane"]`;
  - `listMartinProjects` hits `/api/martin/stave/projects?query=alpha&limit=10` with `Authorization: Bearer stc_x`, parses the `valid-project-list.json` fixture (`{ contract, projects: [13-field rows] }`) and maps rows through `toMartinProjectSummary`; unknown extra key in a project → throws `invalid_response`;
  - `getMartinContextBundle` parses `valid-context-bundle.json`; a response with `content-length` header of `600_000` throws `AtelierConnectorHttpError` code `response_too_large`; a `~100KB` markdown body under the 512KB bound succeeds (proves the per-endpoint bound);
  - `postMartinEvents` posts `{ contract: "stave-sync-v1", events }` matching `StaveSyncEventsRequestV1Schema` and returns the parsed per-item `results` (`status`); >20 events throws before any fetch;
  - `mergeMartinLinks` posts `{ contract: "stave-sync-v1", links }` and aggregates the server's `results[].action` into `{ ok: true, inserted, updated, skipped }` counts;
...
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-http-client.test.ts`; expect module-not-found for `../electron/main/atelier-connector/http-client`.
- [ ] **Step 3: Write minimal implementation** — copy the crane `request<T>()` core (`electron/main/crane-connector/http-client.ts:212`) verbatim semantics: `AbortSignal.timeout(30_000)` merged with caller signal, `redirect: "error"`, `cache: "no-store"`, `readBoundedJson` parameterized as `maxResponseBytes` per call — `24_000` default, `CONTEXT_BUNDLE_MAX_RESPONSE_BYTES = 524_288` (512KB) for `getMartinContextBundle`, `64_000` for list/events/links responses. `ErrorResponseSchema` identical to crane's. Exchange path stays `/api/crane/stave/connectors/exchange` (spec §4.1 keeps the path); body adds `requestedScopes`. Response schema: crane `ExchangeResponseSchema` with its `connector` metadata object extended by `scopes: z.array(AtelierConnectorScopeSchema).min(1).max(2).optional()` → client defaults to `["crane"]` when absent. Martin paths: `` `/api/martin/stave/projects` ``, `` `/api/martin/stave/projects/${encodeURIComponent(projectRef)}/context-bundle` ``, `.../events`, `.../links/merge`. Validate outbound payloads with `StaveSyncEventsRequestV1Schema.parse` / `StaveSyncLinksMergeRequestV1Schema.parse` before sending.
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-http-client.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add electron/main/atelier-connector/http-client.ts tests/martin-http-client.test.ts && git commit -m "feat(atelier-connector): http client for pairing scopes and martin sync endpoints" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`

---

### Task 4: Durable outbox SQLite store `martin_sync_outbox`

**Files:**
- Create: `electron/persistence/martin-sync-outbox-store.ts`
...
- Test: `tests/martin-sync-outbox-store.test.ts`
...
```ts
export type MartinOutboxKind = "event" | "links_merge";
export type MartinOutboxStatus = "pending" | "delivered" | "failed" | "held";
export interface MartinOutboxEntry {
  id: string; workspaceId: string; projectRef: string;
  kind: MartinOutboxKind; payloadJson: string;
  attempts: number; nextAttemptAt: string; createdAt: string;
  deliveredAt: string | null; status: MartinOutboxStatus;
}
export class MartinSyncOutboxStore {
  constructor(database: unknown);                       // bootstrap() creates table + indexes
  enqueue(input: { workspaceId: string; projectRef: string; kind: "event"; payloadJson: string; now: string }): MartinOutboxEntry;
  upsertLinksMerge(input: { workspaceId: string; projectRef: string; payloadJson: string; nextAttemptAt: string; now: string }): MartinOutboxEntry;
  listDue(args: { now: string; limit: number }): MartinOutboxEntry[];   // status='pending' AND next_attempt_at <= now, ordered created_at ASC
...
- [ ] **Step 1: Write the failing test** — model `tests/crane-job-binding-store.test.ts` (`import { Database } from "bun:sqlite"`, `new MartinSyncOutboxStore(new Database(":memory:"))`):
...
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-sync-outbox-store.test.ts`; expect module-not-found.
...
Row ids via `randomUUID()`. `upsertLinksMerge`: `UPDATE ... WHERE workspace_id = ? AND kind = 'links_merge' AND status IN ('pending','held')`; if `changes === 0`, INSERT. Parse rows through a `MartinOutboxEntrySchema` Zod strict object (crane store pattern `parseBindingRow`). In `sqlite-store.ts` add `private martinSyncOutbox: MartinSyncOutboxStore;` + `this.martinSyncOutbox = new MartinSyncOutboxStore(this.db);` (constructor, next to line 229) and delegates `enqueueMartinOutboxEntry`, `upsertMartinLinksMergeEntry`, `listDueMartinOutboxEntries`, `markMartinOutboxDelivered`, `markMartinOutboxRetry`, `markMartinOutboxFailed`, `setMartinOutboxWorkspaceHeld`, `retryFailedMartinOutboxEntries`, `countMartinOutbox`, `pruneMartinOutboxDeliveredBefore` (next to the crane delegates at ~line 2845).
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-sync-outbox-store.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add electron/persistence tests/martin-sync-outbox-store.test.ts && git commit -m "feat(martin-sync): durable sqlite outbox store" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Files:**
- Create: `electron/main/martin-sync/runtime.ts`
- Create: `electron/main/martin-sync/service.ts`
- Modify: `electron/main.ts` (~line 39-43: add `stopMartinSyncRuntime()` to `runBeforeQuitCleanup`)
- Test: `tests/martin-sync-runtime.test.ts`
...
export class MartinSyncRuntime {
  constructor(dependencies: MartinSyncRuntimeDependencies);
...
// service.ts
export function getMartinSyncRuntime(): MartinSyncRuntime;
export function configureMartinSync(settings: MartinSyncSettings): MartinSyncPublicStatus;
export function getMartinSyncStatus(): MartinSyncPublicStatus;
export function enqueueMartinSyncEvent(args: EnqueueEventArgs): void;
export function noteMartinWorkspaceLinksChanged(args: LinksChangedArgs): void;
...
interface MartinSyncRuntimeDependencies {
  persistence: MartinSyncOutboxPersistence;          // sqlite delegate subset
...
- [ ] **Step 1: Write the failing test** — model `tests/crane-connector-runtime.test.ts` harness: injected timers array, fixed `now`, fake persistence backed by an in-memory `Map`, fake client with programmable failures:
  ...
  - after `MAX_MARTIN_SYNC_ATTEMPTS` (8) failures → `markFailed`, status counts `failedCount: 1`; `retryFailed()` requeues and drains again;
  ...
  - coalescing: two `noteLinksChanged` calls 5 "seconds" apart (advance fake now) produce a single `links_merge` row with the **second** payload, and it is not due until 30s after the second call (`MARTIN_LINKS_MERGE_DEBOUNCE_MS = 30_000`); after the debounce, one `mergeMartinLinks` call with the latest links;
  - events for the same projectRef are batched ≤ 20 per `postMartinEvents` call.
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-sync-runtime.test.ts`; expect module-not-found.
- [ ] **Step 3: Write minimal implementation** — runtime clones the crane concurrency skeleton (`electron/main/crane-connector/runtime.ts`): `private operationQueue: Promise<void>`, `private generation = 0`, `private timer`, `enqueue()` / `schedule(delayMs)` / `stopPendingWork()` copied semantics (lines 1180-1214). Drain algorithm inside `enqueue(() => this.drain())`:
  1. If `!settings.enabled` → status `disabled`, return. `getCredential()` null or missing `"martin"` scope → status `unpaired`, return (no timer).
  ...
  3. Group `kind === "event"` rows by `projectRef` into ≤20 chunks → `postMartinEvents`; each result → `markDelivered`. Each `kind === "links_merge"` row → parse payload `{ links }` → `mergeMartinLinks` → `markDelivered`. Exhaustive switch on `kind` with `never` default.
  ...
  - `enqueueEvent`: `StaveSyncEventV1Schema.parse(event)` → `persistence.enqueueMartinOutboxEntry({ ..., payloadJson: JSON.stringify(event), now })` → `schedule(0)` when enabled.
  - `noteLinksChanged`: `upsertMartinLinksMergeEntry({ ..., payloadJson: JSON.stringify({ links }), nextAttemptAt: iso(now + 30_000) })` → `schedule(30_000)`; the row's `next_attempt_at` implements the trailing debounce so it survives restarts.
  - `service.ts` mirrors `electron/main/crane-connector/service.ts`: lazy singleton, `sendToRenderer("martin-sync:status", status)` and `sendToRenderer("martin-sync:mapping-stale", payload)` via `getMainWindow()`. `electron/main.ts`: add `import { stopMartinSyncRuntime } from "./main/martin-sync/service";` and `Promise.resolve(stopMartinSyncRuntime()),` inside the `Promise.allSettled` at line 39.
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-sync-runtime.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add electron/main/martin-sync electron/main.ts tests/martin-sync-runtime.test.ts && git commit -m "feat(martin-sync): outbox runtime with backoff, dead-letter, and link coalescing" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`

---

### Task 6: `WorkspaceInformationState.martinProject` + host-service set action (4-step recipe, part 1)
...
- Test: `tests/martin-workspace-information.test.ts`
...
- Host-service: `setWorkspaceMartinProject(args: { workspaceId: string; project: WorkspaceMartinProjectLink | null }): Promise<WorkspaceInformationMutationResult>` — implemented with `updateWorkspaceInformationState` so it persists and emits `local-mcp.workspace-information-updated`.
- Protocol: `| "set-workspace-martin-project"` appended to `HostLocalMcpAction`.
- `stave-mcp-service.ts`: same-named async wrapper via `invokeLocalMcp`.
- Kickoff: `KickoffProposalDraft` gains `martinProject?: WorkspaceMartinProjectLink | null`; `buildWorkspaceInformationSeed` copies it into the seed when present.

- [ ] **Step 1: Write the failing test** — `tests/martin-workspace-information.test.ts`:
  - `createEmptyWorkspaceInformation().martinProject` is `null`;
  - `buildWorkspaceInformationSeed({ ...draft, martinProject: link })` carries the link into the returned state; without it, seed keeps `null` (build a minimal `KickoffProposalDraft` with empty `panelEntries`/`todos`/`notes` — mirror the draft shape returned by the deterministic fallback at `workspace-kickoff.ts:553-565`).
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-workspace-information.test.ts`; expect a type/compile error (`martinProject` does not exist) or `undefined !== null` assertion failure.
- [ ] **Step 3: Write minimal implementation** — add the interface + field; set `martinProject: null` in `createEmptyWorkspaceInformation`. Host-service function:
...
Add the dispatch case in `electron/host-service.ts` (`case "set-workspace-martin-project": ... localMcpRuntime.setWorkspaceMartinProject(args as never)` — match the casting convention of adjacent cases). Add the `stave-mcp-service.ts` wrapper following the `replaceWorkspaceNotes` template (lines 81-89). Renderer display works automatically through the existing `local-mcp:workspace-information-updated` → `applyExternalWorkspaceInformationUpdate` path.
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-workspace-information.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add src/lib/workspace-information.ts src/lib/workspace-kickoff.ts electron/host-service tests/martin-workspace-information.test.ts electron/host-service.ts electron/main/stave-mcp-service.ts && git commit -m "feat(martin-sync): first-class martinProject field on workspace information" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Files:**
- Create: `electron/main/martin-sync/project-link.ts`
- Create: `electron/main/martin-sync/context-snapshot.ts`
- Create: `src/lib/martin-sync/staleness.ts`
- Modify: `electron/main/martin-sync/service.ts` (add credential accessor + `local-mcp.workspace-information-updated` subscription)
- Test: `tests/martin-context-snapshot.test.ts` (staleness + snapshot path/write)

**Interfaces:**
- Consumes: `setWorkspaceMartinProject`, `getWorkspaceInformation`, `listKnownProjects` from `electron/main/stave-mcp-service.ts`; `AtelierConnectorHttpClient` + vault; `buildMartinSyncLinks`; `onHostServiceEvent` from `electron/main/host-service-client.ts:487`; runtime service (Task 5).
- Produces:

```ts
// src/lib/martin-sync/staleness.ts
export const MARTIN_CONTEXT_MAX_AGE_MS = 60 * 60 * 1_000;
export function isMartinContextStale(args: { lastPulledAt: string | null; now?: Date; maxAgeMs?: number }): boolean;

// electron/main/martin-sync/context-snapshot.ts
export function buildMartinContextSnapshotRelativePath(slug: string): string; // ".stave/context/martin/<slug>.md"
export async function writeMartinContextSnapshot(args: { workspacePath: string; slug: string; markdown: string }): Promise<{ absolutePath: string; relativePath: string }>;

// electron/main/martin-sync/project-link.ts
export async function listMartinProjects(args: { query?: string; limit?: number }): Promise<MartinProjectSummary[]>;
export async function linkMartinProject(args: { workspaceId: string; projectRef: string }): Promise<{ project: WorkspaceMartinProjectLink; snapshotRelativePath: string }>;
export async function unlinkMartinProject(args: { workspaceId: string }): Promise<{ ok: true }>;
export async function refreshMartinContext(args: { workspaceId: string }): Promise<{ project: WorkspaceMartinProjectLink; snapshotRelativePath: string; markdown: string }>;
```

- [ ] **Step 1: Write the failing test** — `tests/martin-context-snapshot.test.ts`:
  - `isMartinContextStale({ lastPulledAt: null })` → `true`; 59 minutes old → `false`; 61 minutes old → `true` (fixed `now`);
  - slug sanitization: `buildMartinContextSnapshotRelativePath("checkout-v2")` → `".stave/context/martin/checkout-v2.md"`; a slug containing `../` or path separators throws (defense: the contract slug regex should already forbid it — assert both);
  - `writeMartinContextSnapshot` to a tmpdir creates nested dirs and round-trips content; a second write replaces content.
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-context-snapshot.test.ts`; expect module-not-found.
...
async function requireMartinClient() {
  ...
  if (!credential.scopes.includes("martin")) throw new Error("scope_missing");
  return {
...
`linkMartinProject`: fetch bundle → resolve workspace path/name/branch via `listKnownProjects()` (flatten `RegisteredProjectInfo.workspaces`, `RegisteredWorkspaceInfo.path/name/branch` — `local-mcp-runtime.ts:116-123`) → `writeMartinContextSnapshot` → `setWorkspaceMartinProject` with `{ ref, slug, name, url, linkedAt: nowIso, lastPulledAt: nowIso }` → if master switch on, `enqueueMartinSyncEvent` `workspace_linked` (summary `` `${workspaceName} (${branch})` ``, `sourceUrl: project.url`) and `noteMartinWorkspaceLinksChanged` with `buildMartinSyncLinks(result.workspaceInformation)`. `unlinkMartinProject`: read current WI for the projectRef, set field to `null`, enqueue `workspace_unlinked`. `refreshMartinContext`: fetch bundle, rewrite snapshot, update `lastPulledAt` (clear `stale`). Service subscription (register once in `getMartinSyncRuntime()` bootstrap):
...
Wire `emitMappingStale` (Task 5) to call `setWorkspaceMartinProject` with `{ ...current, stale: true }` (read-modify via `getWorkspaceInformation`).
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-context-snapshot.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add electron/main/martin-sync src/lib/martin-sync/staleness.ts tests/martin-context-snapshot.test.ts && git commit -m "feat(martin-sync): project link lifecycle, context snapshot pull, staleness helper" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Files:**
- Create: `electron/main/ipc/atelier-connector.ts`, `electron/main/ipc/martin-sync.ts`
...
- IPC channels (all `ipcMain.handle`, args validated with `safeParse`, safe error messages — clone `registerCraneConnectorHandlers` shape from `electron/main/ipc/crane-connector.ts`):
  ...
  - `martin-sync:get-status`, `martin-sync:configure` (`MartinSyncConfigureArgsSchema`), `martin-sync:enqueue` (`MartinSyncEnqueueArgsSchema` — builds the wire event in main: `staveEventId: randomUUID()`, `tier` derived from kind exhaustively: `work_update → "interpretive"`, others `"factual"`; the top-level `{ contract, events }` wrapper is added by the HTTP client), `martin-sync:links-changed` (`MartinSyncLinksChangedArgsSchema`), `martin-sync:retry-failed`, `martin-sync:list-projects`, `martin-sync:link-project`, `martin-sync:unlink-project`, `martin-sync:refresh-context`
  - push events: `martin-sync:status`, `martin-sync:mapping-stale` (emitted by Task 5 service)
- Preload `window.api.atelierConnector = { getStatus, pair }` and `window.api.martinSync = { getStatus, configure, enqueue, notifyLinksChanged, retryFailed, listProjects, linkProject, unlinkProject, refreshContext, subscribeStatus, subscribeMappingStale }` — invoke/subscribe shapes copied from the `craneConnector` bridge (preload lines 1466-1532).
- `src/types/window-api.d.ts`: `WindowAtelierConnectorApi`, `WindowMartinSyncApi` with all members optional-function style matching `WindowCraneConnectorApi` (lines 702-744), registered as `atelierConnector?:` / `martinSync?:`.

- [ ] **Step 1: Write the failing test** — this is contract glue; the test asserts the arg schemas exported through `electron/main/ipc/schemas.ts` (same role as `crane-stave-contract.test.ts` lines importing from `../electron/main/ipc/schemas`). Add to `tests/martin-sync-contract.test.ts`:
...
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-sync-contract.test.ts`; expect export-not-found from `../electron/main/ipc/schemas`.
- [ ] **Step 3: Write minimal implementation** — re-export block in `schemas.ts`; two handler files with `safeConnectorErrorMessage`-style sanitizers (map `unauthorized|forbidden` → "Atelier rejected this connector. Pair it again.", `network_unavailable` → "Atelier is currently unreachable.", never echo raw errors); register `registerAtelierConnectorHandlers(); registerMartinSyncHandlers();` in `electron/main/ipc/index.ts`; preload bridge + window-api types exactly mirroring the crane pattern (including the subscriber `Set` + `ipcRenderer.on("martin-sync:status", ...)` fanout at the top of preload near line 234). Per `docs/architecture/contracts.md` Window API checklist: verify `electron/preload.ts`, `src/types/window-api.d.ts`, `electron/main/ipc/*`, and renderer call sites stay aligned.
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-sync-contract.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add electron/main/ipc electron/preload.ts src/types/window-api.d.ts tests/martin-sync-contract.test.ts && git commit -m "feat(martin-sync): ipc handlers, preload bridge, and window api contract" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Interfaces:**
- Consumes: `listMartinProjects`, `linkMartinProject`, `unlinkMartinProject`, `refreshMartinContext` from `electron/main/martin-sync/project-link.ts`; `toStructuredResult` (line 88).
- Produces MCP tools: `stave_martin_list_projects`, `stave_martin_link_project`, `stave_martin_unlink_project`, `stave_martin_get_context`.
...
- [ ] **Step 2: Verify absence** — `grep -c "stave_martin" electron/main/stave-mcp-server.ts` returns 0.
...
Import the four functions at the top of `stave-mcp-server.ts` from `"./martin-sync/project-link"`.
- [ ] **Step 4: Verify** — `bun run typecheck` passes and `grep -c "stave_martin" electron/main/stave-mcp-server.ts` returns ≥ 4.
- [ ] **Step 5: Commit** — `git add electron/main/stave-mcp-server.ts && git commit -m "feat(martin-sync): register martin project mcp tools" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`

---

### Task 10: AppSettings.martinSync + rehydrate normalization + SettingDefinition + configure push
...
- Modify: `src/App.tsx` (extend the crane connector effect at lines 119-204 with a parallel martin push)
- Test: extend `tests/martin-sync-settings.test.ts`

**Interfaces:**
- `AppSettings` gains `/** Martin workspace sync toggles. Secrets stay in the Electron main vault. */ martinSync: MartinSyncSettings;`; `defaultSettings` gains `martinSync: { ...DEFAULT_MARTIN_SYNC_SETTINGS },`.
- Rehydrate: `state.settings.martinSync = normalizeMartinSyncSettings(raw.martinSync);`.
- Registry entry:
...
```ts
{
  ...
  keywords: ["martin", "atelier", "sync", "project", "events", "links", "outbox", "connector"],
...
- `App.tsx`: inside the existing connector effect, `pushMartinConfig(settings)` calling `window.api?.martinSync?.configure?.(normalizeMartinSyncSettings(settings.martinSync))` on mount and whenever any of the five booleans change in the store subscription (compare normalized prev/next like lines 174-204).

- [ ] **Step 1: Write the failing test** — add to `tests/martin-sync-settings.test.ts`:
...
test("app settings carry martin sync defaults", () => {
  expect(defaultSettings.martinSync).toEqual(DEFAULT_MARTIN_SYNC_SETTINGS);
});

test("martin sync setting definition is sensitive and excluded from export", () => {
  const definition = settingDefinitions.find((d) => d.key === "martinSync");
...
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-sync-settings.test.ts`; expect `martinSync` undefined / definition not found.
- [ ] **Step 3: Write minimal implementation** — the four file edits above.
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-sync-settings.test.ts && bun run typecheck`.
- [ ] **Step 5: Commit** — `git add src/store/app-settings.ts src/store/app-store-persistence.ts src/components/layout/settings-dialog.registry.ts src/App.tsx tests/martin-sync-settings.test.ts && git commit -m "feat(martin-sync): app settings, rehydrate normalization, and setting definition" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Files:**
- Create: `src/lib/martin-sync/renderer-triggers.ts`
...
- Test: `tests/martin-renderer-triggers.test.ts`

**Interfaces:**
- Produces (all read `window.api?.martinSync` lazily; accept a minimal structural state to avoid store import cycles):
...
}): void;      // computes buildMartinSyncLinks for both; if JSON differs → window.api.martinSync.notifyLinksChanged
export function maybeRefreshMartinContext(args: {
  workspaceId: string;
  martinProject: WorkspaceMartinProjectLink | null | undefined;
}): void;      // isMartinContextStale → window.api.martinSync.refreshContext (fire-and-forget)
```

- [ ] **Step 1: Write the failing test** — `tests/martin-renderer-triggers.test.ts` for the pure/decision surface (stub `window.api` on `globalThis` with a recording fake, following existing renderer tests that stub `window`):
  - `shouldPushMartinEvent`: master off → always false; master on → `pr_opened`/`task_completed` true by default, `work_update` false, `workspace_linked` true regardless of individual toggles;
  - `notifyMartinPrOpened` with a linked context calls `enqueue` once with `kind: "pr_opened"`, `summary: prTitle`, `sourceUrl: prUrl`, `projectRef` from `martinProject.ref`; with `martinProject: null` or `stale: true` it does not call;
  - `notifyMartinInformationEdited` with identical links arrays makes no call; with a new PR added calls `notifyLinksChanged` with the mapped links;
  - `maybeRefreshMartinContext` calls `refreshContext` only when stale.
- [ ] **Step 2: Run test to verify it fails** — `bun test tests/martin-renderer-triggers.test.ts`; expect module-not-found.
...
if (args.trigger === "task.archiving" && args.taskTitle) {
  notifyMartinTaskArchived({
    context: collectMartinTriggerContext(get(), args.workspaceId),
    settings: get().settings.martinSync,
    taskTitle: args.taskTitle,
  });
}
```

  where `collectMartinTriggerContext(state, workspaceId)` (exported from renderer-triggers.ts, accepts a structural `{ workspaces, workspaceBranchById, activeWorkspaceId, workspaceInformation, workspaceRuntimeCacheById }`) resolves name/branch/WI for active or cached workspaces via the same lookup `applyWorkspaceTurnSummaryToState` uses.
  - `app.store.ts` `applyWorkspaceTurnSummaryToState` after the `if (!didUpdate) return;` guard (line 878): `notifyMartinTurnSummary({ context: collectMartinTriggerContext(get(), args.workspaceId), settings: get().settings.martinSync, workSummary: args.summary.workSummary });`
  - `TopBarOpenPR.tsx` after the PR-created toast block (line ~1475): `notifyMartinPrOpened({ context: ..., settings: ..., prUrl: prResult.prUrl ?? "", prTitle: dialogTitle })` guarded on `prResult.prUrl` (use the component's PR title state variable — confirm exact name in file).
  - `app-store-task-core-actions.ts` `updateWorkspaceInformation` (line 414): capture `previous = state.workspaceInformation` before `set`, then after `set` call `notifyMartinInformationEdited` with previous/next and the active workspace context.
  - `app-store-workspace-management-actions.ts` `switchWorkspace`: at the end of the successful switch (after workspace information is in place), `maybeRefreshMartinContext({ workspaceId, martinProject: get().workspaceInformation.martinProject });`
- [ ] **Step 4: Run test to verify it passes** — `bun test tests/martin-renderer-triggers.test.ts && bun run typecheck && bun run check:max-lines-ratchet`.
- [ ] **Step 5: Commit** — `git add src/lib/martin-sync/renderer-triggers.ts src/store/app.store.ts src/store/app-store-task-core-actions.ts src/store/app-store-workspace-management-actions.ts src/components/layout/TopBarOpenPR.tsx tests/martin-renderer-triggers.test.ts && git commit -m "feat(martin-sync): wire renderer lifecycle triggers into the sync outbox" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`

---

### Task 12: Settings UI card + Information panel Martin card

**Files:**
- Create: `src/components/layout/settings-dialog-martin-sync.tsx`
- Create: `src/components/layout/WorkspaceInformationMartinCard.tsx`
...
**Interfaces:**
- Consumes: `window.api.atelierConnector` / `window.api.martinSync` (Task 8), `useAppStore` settings + `updateSettings({ patch })` (pattern at `settings-dialog-sections.tsx:3930`), UI kit from `@/components/ui` (see imports of `settings-dialog-crane-connector.tsx:12-24`).
- Produces: `MartinSyncSettingsSection` (Atelier connector status: paired/scopes/last seen, pair form with base URL + `stp_` code + scope checkboxes, master + 4 toggles, outbox pending/failed counts, "Retry failed" button) and `WorkspaceInformationMartinCard` (linked: project name → `window.api.shell/openExternal` on `project.url`, "Last pulled …", Refresh + Unlink buttons, stale warning badge; unlinked: query input + `listProjects` results + Link button).
...
export function MartinSyncSettingsSection() {
  ...
  useEffect(() => {
    void window.api?.martinSync?.getStatus?.().then((r) => r && setStatus(r.status));
    void window.api?.atelierConnector?.getStatus?.().then((r) => r && setConnector(r.status));
    return window.api?.martinSync?.subscribeStatus?.(setStatus);
  }, []);

  const patch = (partial: Partial<MartinSyncSettings>) =>
    updateSettings({ patch: { martinSync: { ...martinSync, ...partial } } });

  return (
    <SettingsCard
      id="settings-field-martin-sync"
      title="Martin sync"
      description="Push workspace events and links to a linked Martin project."
    >
      ...
      <SettingsToggleRow title="Enable Martin sync" checked={martinSync.enabled}
        onCheckedChange={(enabled) => patch({ enabled })} />
      <SettingsToggleRow title="PR opened events" checked={martinSync.prOpened}
        onCheckedChange={(prOpened) => patch({ prOpened })} />
      <SettingsToggleRow title="Task completed events" checked={martinSync.taskCompleted}
        onCheckedChange={(taskCompleted) => patch({ taskCompleted })} />
      <SettingsToggleRow title="Resource link mirroring" checked={martinSync.resourceLinks}
        onCheckedChange={(resourceLinks) => patch({ resourceLinks })} />
      <SettingsToggleRow title="Turn summaries (interpretive)" checked={martinSync.turnSummaries}
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        {(status?.failedCount ?? 0) > 0 ? (
          <Button size="sm" variant="outline"
            onClick={() => void window.api?.martinSync?.retryFailed?.()}>
            Retry failed
          </Button>
        ) : null}
      </div>
...
`WorkspaceInformationMartinCard` reads `useAppStore((s) => s.workspaceInformation.martinProject)` and `activeWorkspaceId`; unlink/refresh call the bridge and rely on the `local-mcp:workspace-information-updated` push to refresh the store; picker keeps `query`/`results` local state.
- [ ] **Step 2: Typecheck** — `bun run typecheck` (also confirms `case "integrations"` stays a valid `SectionId` switch).
- [ ] **Step 3: Manual verification** — `bun run dev` (or the repo's electron dev script from `package.json`), open Settings → Integrations: both cards render, toggles persist across reload (rehydrate normalization), pair form disabled without secure storage message parity with the crane card; open a workspace → Information panel shows the Martin card in unlinked state.
- [ ] **Step 4: Re-run gates** — `bun run typecheck && bun test tests/martin-sync-settings.test.ts`.
- [ ] **Step 5: Commit** — `git add src/components/layout/settings-dialog-martin-sync.tsx src/components/layout/settings-dialog-sections.tsx src/components/layout/WorkspaceInformationMartinCard.tsx src/components/layout/WorkspaceInformationPanel.tsx && git commit -m "feat(martin-sync): settings card and information panel project card" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
**Files:**
- Create: `docs/features/martin-sync.md` (from `docs/templates/feature-guide-template.md`)
- Modify (if publishing publicly): `site/src/public-docs.ts`

**Interfaces:** none (documentation).

- [ ] **Step 1: Write the doc** — sections per template: Summary (link a Stave workspace to a Martin project; factual events pushed automatically, turn summaries opt-in; context snapshot at `.stave/context/martin/<slug>.md`), When To Use It, Before You Start (Atelier connector paired with the `martin` scope — re-pair if the connector predates scopes; OS credential encryption required), Quick Start (Settings → Integrations → pair → enable → link from the Information panel), How It Works (outbox survives offline/quit; dead-lettered items visible in settings with Retry), Limitations (no reverse-merge of Martin links into Stave resources; deleted/archived projects mark the mapping stale and hold pushes), Privacy (only workspace name, branch, titles, URLs, and opt-in summaries are sent — never file paths, diffs, or transcripts). Every file path referenced in the doc must exist (`check:doc-paths`).
- [ ] **Step 2: Verify doc gate** — `bun run check:doc-paths`.
- [ ] **Step 3: Full focused suite** — `bun test tests/martin-sync-contract.test.ts tests/martin-sync-settings.test.ts tests/martin-atelier-vault.test.ts tests/martin-http-client.test.ts tests/martin-sync-outbox-store.test.ts tests/martin-sync-runtime.test.ts tests/martin-workspace-information.test.ts tests/martin-context-snapshot.test.ts tests/martin-renderer-triggers.test.ts tests/crane-connector-runtime.test.ts tests/crane-connector-credential-vault.test.ts tests/crane-stave-contract.test.ts`
...
- [ ] **Step 5: Commit** — `git add docs/features/martin-sync.md site/src/public-docs.ts && git commit -m "docs(martin-sync): add feature guide" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"`
...
6. **`src/lib/atelier-connector/types.ts` is used for connector-scope/pairing types** alongside `src/lib/martin-sync/` (explicitly allowed by the task brief) so the renderer settings card can type connector status without importing martin contract code.
7. **Kickoff seed inclusion is a pass-through field** (`KickoffProposalDraft.martinProject?`) — no kickoff classifier work; nothing currently produces a linked draft, but the seed path (spec §5.4) is ready for it.
...
- <stave-worktree>/electron/host-service/local-mcp-runtime.ts (`updateWorkspaceInformationState` line 731 — the single WI mutation path the new `set-workspace-martin-project` action and MCP tools flow through)
